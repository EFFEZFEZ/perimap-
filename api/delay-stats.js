...
export default async function handler(req, res) {
  ...
  try {
    ...
    const lineStats = await sql`
      ...
      ${line ? sql`AND line_code = ${line}` : sql``}
      GROUP BY line_code
    ...
    const hourlyStats = await sql`
      ...
      WHERE recorded_at >= NOW() - INTERVAL '${daysInt} days'
      ${line ? sql`AND line_code = ${line}` : sql``}
      ${hour ? sql`AND hour_of_day = ${parseInt(hour)}` : sql``}
    ...
    if (detailed === 'true') {
      const recentQuery = await sql`
        ...
        LIMIT 100;
      `;
      recentReports = recentQuery.rows;
    }
    ...
    const dayOfWeekFormatted = dayOfWeekStats.rows.map(row => ({
      ...row,
