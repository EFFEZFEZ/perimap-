...
/**
 * DelayDetectionManager - Système de détection des retards
 * Analyse les positions GPS crowdsourcées vs horaires GTFS prévus
 */
const DelayDetectionManager = (function() {
    'use strict';

    const CONFIG = {
        // Seuil de retard significatif (en secondes)
        DELAY_THRESHOLD: 120, // 2 minutes
        
        // Seuil de retard important (en secondes)
        MAJOR_DELAY_THRESHOLD: 300, // 5 minutes
        
        // Intervalle de vérification des retards (en ms)
        CHECK_INTERVAL: 30000, // 30 secondes
        
        // Vitesse moyenne bus en m/s (hors pointe)
        AVG_BUS_SPEED: 8.33, // ~30 km/h
        
        // Multiplicateur vitesse en heure de pointe
        PEAK_HOUR_MULTIPLIER: 2, // vitesse divisée par 2
        
        // Stockage des alertes
        ALERTS_STORAGE_KEY: 'peribus_delay_alerts',
        
        // Durée de vie d'une alerte (en ms)
        ALERT_TTL: 30 * 60 * 1000 // 30 minutes
    };

    // État des retards
    let state = {
        delayedTrips: new Map(), // tripId -> {delaySeconds, routeId, routeName, lastUpdate}
        alertListeners: [],
        checkIntervalId: null
    };

    /**
     * Initialise le système de détection de retards
     */
    function init() {
        loadSavedAlerts();
        startDelayChecking();
        console.log('⏰ Système de détection de retards initialisé');
    }
    ...
    function loadSavedAlerts() {
        try {
            const saved = localStorage.getItem(CONFIG.ALERTS_STORAGE_KEY);
            if (saved) {
                const alerts = JSON.parse(saved);
                const now = Date.now();
                // Filtrer les alertes expirées
                for (const [tripId, alert] of Object.entries(alerts)) {
                    if (now - alert.lastUpdate < CONFIG.ALERT_TTL) {
                        state.delayedTrips.set(tripId, alert);
                    }
                }
            }
        } catch (e) {
            console.warn('Erreur chargement alertes retard:', e);
        }
    }
    ...
    function saveAlerts() {
        try {
            const alertsObj = {};
            state.delayedTrips.forEach((alert, tripId) => {
                alertsObj[tripId] = alert;
            });
            localStorage.setItem(CONFIG.ALERTS_STORAGE_KEY, JSON.stringify(alertsObj));
        } catch (e) {
            console.warn('Erreur sauvegarde alertes retard:', e);
        }
    }

    /**
     * Démarre la vérification périodique des retards
     */
    function startDelayChecking() {
        if (state.checkIntervalId) return;
    ...
    /**
     * Vérifie tous les retards potentiels
     */
    async function checkAllDelays() {
        if (typeof CrowdsourcingManager === 'undefined') return;

        const crowdState = CrowdsourcingManager.getState();
        if (!crowdState.contributors || crowdState.contributors.size === 0) return;

        console.log('🔍 Vérification des retards...', crowdState.contributors.size, 'trips suivis');

        for (const [tripId, positions] of crowdState.contributors) {
            if (positions.length < 2) continue;
            
            try {
                const delayInfo = await analyzeDelay(tripId, positions);
                if (delayInfo) {
                    updateDelayStatus(tripId, delayInfo);
                }
            } catch (e) {
                console.warn('Erreur analyse retard pour trip', tripId, e);
            }
        }
    ...
    /**
     * Analyse le retard d'un trip basé sur ses positions
     */
    async function analyzeDelay(tripId, positions) {
        if (!positions || positions.length < 2) return null;

        const latestPos = positions[positions.length - 1];
        const now = Date.now();

        // Obtenir les infos du trip depuis GTFS
        const tripInfo = await getTripInfo(tripId);
        if (!tripInfo) return null;

        // Trouver le prochain arrêt prévu
        const nextStop = await findNextScheduledStop(tripId, latestPos);
        if (!nextStop) return null;

        // Calculer la distance jusqu'au prochain arrêt
        const distanceToStop = CrowdsourcingManager.haversineDistance(
        ...
        const isPeakHour = isCurrentlyPeakHour();
        const effectiveSpeed = isPeakHour 
            ? CONFIG.AVG_BUS_SPEED / CONFIG.PEAK_HOUR_MULTIPLIER 
            : CONFIG.AVG_BUS_SPEED;
        
        const estimatedTravelTime = distanceToStop / effectiveSpeed; // en secondes
        const estimatedArrivalTime = now + (estimatedTravelTime * 1000);

        // Comparer avec l'heure prévue
        const scheduledArrivalTime = nextStop.scheduledArrival;
        const delaySeconds = Math.floor((estimatedArrivalTime - scheduledArrivalTime) / 1000);

        // Si en retard significatif
        if (delaySeconds > CONFIG.DELAY_THRESHOLD) {
            return {
                tripId,
                ...
                delaySeconds,
                isMajorDelay: delaySeconds > CONFIG.MAJOR_DELAY_THRESHOLD,
                nextStopId: nextStop.stopId,
                nextStopName: nextStop.stopName,
                scheduledTime: formatTime(new Date(scheduledArrivalTime)),
                estimatedTime: formatTime(new Date(estimatedArrivalTime)),
                position: latestPos,
                lastUpdate: now,
                isPeakHour
            };
    ...
    /**
     * Obtient les infos d'un trip depuis GTFS
     */
    async function getTripInfo(tripId) {
        if (typeof dataManager !== 'undefined' && dataManager.getTripInfo) {
            return await dataManager.getTripInfo(tripId);
        }

        // Fallback: chercher dans les données globales
        if (typeof window.tripsData !== 'undefined') {
            const trip = window.tripsData.find(t => t.trip_id === tripId);
            if (trip) {
                const route = window.routesData?.find(r => r.route_id === trip.route_id);
                return {
                    tripId: trip.trip_id,
                    routeId: trip.route_id,
                    routeName: route?.route_short_name || route?.route_long_name || 'Ligne',
    ...
    /**
     * Trouve le prochain arrêt prévu pour un trip
     */
    async function findNextScheduledStop(tripId, currentPosition) {
        // Obtenir les stop_times du trip
        let stopTimes = [];
        
        if (typeof dataManager !== 'undefined' && dataManager.getStopTimesForTrip) {
            stopTimes = await dataManager.getStopTimesForTrip(tripId);
        } else if (typeof window.stopTimesData !== 'undefined') {
            stopTimes = window.stopTimesData.filter(st => st.trip_id === tripId);
        }
        ...
        // Trouver le prochain arrêt (celui qui n'est pas encore passé)
        for (const st of stopTimes) {
            const arrivalSeconds = parseTimeToSeconds(st.arrival_time);
            
            // Si l'arrêt est dans le futur
            if (arrivalSeconds > currentTimeSeconds) {
                // Obtenir les coordonnées de l'arrêt
                const stopInfo = await getStopInfo(st.stop_id);
                if (stopInfo) {
                    return {
                        stopId: st.stop_id,
                        ...
                        scheduledArrival: getDateFromTimeString(st.arrival_time),
    ...
    /**
     * Obtient les infos d'un arrêt
     */
    async function getStopInfo(stopId) {
        if (typeof dataManager !== 'undefined' && dataManager.getStopInfo) {
            return await dataManager.getStopInfo(stopId);
        }

        if (typeof window.stopsData !== 'undefined') {
            return window.stopsData.find(s => s.stop_id === stopId);
        }
    ...
    /**
     * Parse une heure GTFS (HH:MM:SS) en secondes
     */
    ...
    /**
     * Convertit une heure GTFS en Date d'aujourd'hui
     */
    function getDateFromTimeString(timeStr) {
        const now = new Date();
        const parts = timeStr.split(':');
        ...
        // Gérer les heures > 24 (service de nuit)
        const daysToAdd = Math.floor(hours / 24);
        hours = hours % 24;

        const result = new Date(now.getFullYear(), now.getMonth(), now.getDate() + daysToAdd, hours, minutes, seconds);
    ...
    /**
     * Met à jour le statut de retard d'un trip
     */
    function updateDelayStatus(tripId, delayInfo) {
        const existing = state.delayedTrips.get(tripId);
        
        // Nouvelle alerte ou mise à jour significative
        if (!existing || Math.abs(existing.delaySeconds - delayInfo.delaySeconds) > 60) {
            state.delayedTrips.set(tripId, delayInfo);
            saveAlerts();
            notifyListeners('delay', delayInfo);
            
            console.log(`🚨 Retard détecté: Ligne ${delayInfo.routeName} - ${Math.floor(delayInfo.delaySeconds / 60)} min de retard`);
            
            // Afficher une notification
            showDelayNotification(delayInfo);
        }
    }
    ...
    function cleanOldAlerts() {
        ...
        toRemove.forEach(tripId => {
            state.delayedTrips.delete(tripId);
            notifyListeners('resolved', { tripId });
        });
    ...
    /**
     * Affiche une notification de retard
     */
    function showDelayNotification(delayInfo) {
        const minutes = Math.floor(delayInfo.delaySeconds / 60);
        const message = `Ligne ${delayInfo.routeName}: ~${minutes} min de retard`;

        // Toast dans l'app
        if (typeof uiManager !== 'undefined' && uiManager.showToast) {
            uiManager.showToast(`⚠️ ${message}`, 'warning');
        }

        // Mettre à jour la bannière d'alerte
        updateAlertBanner();
    }

    /**
     * Met à jour la bannière d'alerte du réseau
     */
    function updateAlertBanner() {
        ...
        if (state.delayedTrips.size === 0) {
            banner.classList.add('hidden');
            banner.classList.remove('delay-alert');
            return;
        }
        ...
        const alerts = [];
        state.delayedTrips.forEach((alert, tripId) => {
            const minutes = Math.floor(alert.delaySeconds / 60);
            alerts.push(`<span class="delay-line" style="--line-color: ${alert.routeColor}">
                <span class="delay-badge">${alert.routeName}</span>
                <span class="delay-time">~${minutes} min</span>
            </span>`);
        });

        content.innerHTML = `
            <span class="delay-icon">⚠️</span>
            <span class="delay-title">Retards en cours:</span>
            ${alerts.join('')}
        `;

        banner.classList.remove('hidden');
        banner.classList.add('delay-alert');
    }

    /**
     * Met à jour l'interface des retards
     */
    ...
    function updateMapMarkers() {
        if (typeof mapRenderer === 'undefined') return;

        state.delayedTrips.forEach((alert, tripId) => {
            if (alert.position) {
                // Ajouter ou mettre à jour le marqueur de retard
                addDelayMarkerToMap(alert);
            }
        });
    }

    /**
     * Ajoute un marqueur de bus en retard sur la carte
     */
    function addDelayMarkerToMap(delayInfo) {
        // Cette fonction sera appelée par mapRenderer
        if (typeof window.addDelayedBusMarker === 'function') {
    ...
    function updateScheduleDisplay() {
        // Mettre à jour les horaires affichés avec les retards
        state.delayedTrips.forEach((alert, tripId) => {
            updateStopTimesWithDelay(tripId, alert.delaySeconds);
        });
    }

    /**
     * Met à jour les horaires d'un trip avec le retard
     */
    function updateStopTimesWithDelay(tripId, delaySeconds) {
        // Chercher tous les éléments d'horaire pour ce trip
        const scheduleElements = document.querySelectorAll(`[data-trip-id="${tripId}"]`);
        
        scheduleElements.forEach(el => {
            const originalTime = el.dataset.originalTime;
            if (originalTime) {
                const newTime = addSecondsToTime(originalTime, delaySeconds);
                el.classList.add('delayed');
                el.innerHTML = `
                    <span class="original-time">${originalTime}</span>
                    <span class="delayed-time">${newTime}</span>
                `;
            }
        });
    }
    ...
    /**
     * Ajoute des secondes à une heure
     */
    function addSecondsToTime(timeStr, seconds) {
        const parts = timeStr.split(':');
        let totalSeconds = parseInt(parts[0]) * 3600 + parseInt(parts[1]) * 60 + (parseInt(parts[2]) || 0);
    ...
    /**
     * Ajoute un listener pour les événements de retard
     */
    function addListener(callback) {
        state.alertListeners.push(callback);
    }
    ...
    function notifyListeners(event, data) {
        state.alertListeners.forEach(callback => {
            try {
                callback(event, data);
            } catch (e) {
                console.warn('Erreur listener retard:', e);
            }
        });
    }

    /**
     * Obtient tous les retards actuels
     */
    function getAllDelays() {
        return Array.from(state.delayedTrips.values());
    }

    /**
     * Obtient le retard pour une ligne spécifique
     */
    function getDelayForRoute(routeId) {
        for (const alert of state.delayedTrips.values()) {
    ...
    /**
     * Calcule les horaires prévus avec retard pour les prochains arrêts
     */
    function getAdjustedSchedule(tripId) {
        const delay = state.delayedTrips.get(tripId);
        if (!delay) return null;

        return {
            tripId,
            delaySeconds: delay.delaySeconds,
            delayMinutes: Math.floor(delay.delaySeconds / 60),
            adjustTime: (originalTime) => addSecondsToTime(originalTime, delay.delaySeconds)
        };
    }
    ...
    return {
        ...
        getAdjustedSchedule,
        addListener,
        isDelayed: (tripId) => state.delayedTrips.has(tripId),
        getDelayInfo: (tripId) => state.delayedTrips.get(tripId)
    };
})();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    DelayDetectionManager.init();
});

// Export pour utilisation dans d'autres modules
if (typeof window !== 'undefined') {
    window.CrowdsourcingManager = CrowdsourcingManager;
    window.DelayDetectionManager = DelayDetectionManager;
}
