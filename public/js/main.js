...
// ?? V60: GTFS Router désactivé temporairement (performances insuffisantes)
...
async function initializeApp() {
    ...
    try {
        ...
        // ?? V60: Router GTFS désactivé temporairement pour performances
        ...
        if (ENABLE_GTFS_ROUTER) {
            ...
            console.log('?? Router GTFS local désactivé (ENABLE_GTFS_ROUTER=false)');
            routerWorkerClient = null;
            routerContext = null;
        }
        ...
        if (geoJsonData) {
            ...
            console.warn('?? Aucun tracé disponible (ni map.geojson ni shapes.txt)');
        }
...
async function executeItinerarySearch(source, sourceElements) {
    ...
    try {
        ...
        // ?? Router GTFS local désactivé (ENABLE_GTFS_ROUTER=false)
            ...
            if (step.travelMode === 'WALK') {
                ...
                if (line) {
                    ...
                    if (shortName === 'TER' || shortName === '322') {
                        ...
                        console.log(`?? V199: Ligne inconnue du GTFS ("${shortName}") mais conservée.`);
                    }
...
function processIntelligentResults(intelligentResults, searchTime) {
    ...
    if (dataManager && dataManager.isLoaded) {
        ...
        if (activeServiceIds.size === 0) {
            console.warn(`?? V194: AUCUN SERVICE ACTIF pour ${dayName} - Les bus ne circulent peut-être pas (GTFS local)`);
            // V199: On ne supprime plus les bus, on fait confiance à Google
            console.log(`?? V199: Conservation des itinéraires Google malgré l'absence de services locaux`);
        } else {
            ...
            const filteredItineraries = itineraries.filter(itin => {
                ...
                if (!hasActiveLine && lineNames.size > 0) {
                    console.log(`?? V199: Lignes [${Array.from(lineNames).join(', ')}] non trouvées dans GTFS local pour ${dayName}, mais conservées (Google First)`);
                    // return false; // V199: On ne filtre plus
                }
                
                // Si pas de nom de ligne trouvé, on garde par prudence
                if (lineNames.size === 0) {
                    console.warn(`?? V194: Itinéraire sans nom de ligne, conservé par défaut`);
                }
                return true;
            });
        ...
        if (searchTime && searchTime.type === 'arriver') {
            ...
            if (dataManager && dataManager.isLoaded) {
                ...
                if (candidateStopIds.size === 0) {
                     console.warn("?? Aucun arrêt candidat trouvé via Google, recherche GTFS impossible.");
                } else {
            ...
            if (missingGtfs.length > 0) {
                console.warn(`?? GTFS: ${missingGtfs.length} départ(s) trouvés dans GTFS mais non proposés par l'API Google.`);
...
function buildFicheHoraireList() {
    ...
    for (const [groupName, routes] of Object.entries(groupedRoutes)) {
        ...
        if (groupName === 'Lignes R') {
            linksHtml = `
                <a href="/data/fichehoraire/grandperigueux_fiche_horaires_ligne_R1_R2_R3_sept_2026.pdf" target="_blank" rel="noopener noreferrer">Lignes R1, R2, R3 La Feuilleraie <> ESAT / Les Gourdoux <> Trélissac Les Garennes / Les Pinots <> P+R Aquacap</a>
                <a href="/data/fichehoraire/grandperigueux_fiche_horaires_ligne_R4_R5_sept_2026.pdf" target="_blank" rel="noopener noreferrer">Lignes R4, R5 Route de Payenché <> Collège Jean Moulin / Les Mondines / Clément Laval <> Collège Jean Moulin</a>
                ...
                <a href="/data/fichehoraire/grandperigueux_fiche_horaires_ligne_R8_R9_sept_2026.pdf" target="_blank" rel="noopener noreferrer">Lignes R8, R9 Jaunour <> Boulazac centre commercial / Stèle de Lesparat <> Place du 8 mai</a>
                <a href="/data/fichehoraire/grandperigueux_fiche_horaires_ligne_R10_R11_sept_2026.pdf" target="_blank" rel="noopener noreferrer">Lignes R10, R11 Notre Dame de Sanilhac poste <> Centre de la communication / Héliodore <> Place du 8 mai</a>
...
function showResultsView() {
    ...
    if (resultsMapRenderer && resultsMapRenderer.map) {
        ...
        setTimeout(() => {
            resultsMapRenderer.map.invalidateSize();
            console.log('??? Carte PC invalidée (300ms)');
        }, 300);
    }
}
