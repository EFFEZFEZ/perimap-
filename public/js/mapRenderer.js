...
export class MapRenderer {
    ...
    initializeMap(useClusters = true) {
        ...
        console.log(`??? Carte ${this.mapElementId} initialisée`);
        this.map.on('click', () => {
    ...
    createBusPopupDomElement() {
        ...
        noticeP.innerHTML = '<strong>?? Données simulées :</strong> Position estimée à partir des horaires théoriques.';
    ...
    async fetchAndUpdateRealtime(masterStop, popup, departuresByLine, currentSeconds, isNextDayDepartures, firstDepartureTime, lat, lon) {
        try {
            ...
            console.warn(`?? Erreur temps réel pour ${masterStop.stop_name}:`, error.message);
        }
    }
